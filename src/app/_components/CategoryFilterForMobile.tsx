...
const CategoryFilterForMobile: React.FC = () => {
  ...
  return (
    <div className="flex flex-col  text-right justify-start gap-2 items-end">
      <span className="text-[0.875rem] text-xs text-black-primary dark:text-dark-white-primary">
        دسته ها
      </span>  {categoryFromUrl &&   <div  onClick={handleRemoveAll} className="flex items-center text-black-primary dark:text-dark-white-primary text-base cursor-pointer  justify-center gap-2">
      ...
      {!categoryFromUrl ? (
        categories.map((category) => (
          <div
            ...
            className="group w-full  flex justify-end p-2 shadow-lg  dark:bg-dark-gray-100 bg-gray-100 items-center gap-2 cursor-pointer"
          >
            <span  className="text-lg  leading-8 text-black-primary dark:text-dark-white-primary font-medium  text-center ">
              {category.name}
            </span>
            <category.icon className="text-xl text-black-primary dark:text-dark-white-primary font-normal"
 />
          </div>
        ))
      ) : (
        ...
        <div className="w-full">
          ...
          {loading ? (
            ...
            uniqueSubCategories1.map((subCategory) => (
              ...
              >
                <span className="text-sm leading-8 text-black-primary dark:text-dark-white-primary font-medium text-right"
 >
                  {subCategory}
                </span>
              </div>
