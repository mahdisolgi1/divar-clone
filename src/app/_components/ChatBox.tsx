...
const ChatBox: React.FC = () => {
    ...
    const [userAds, setUserAds] = useState<{ id: number; title: string; img1: string; userEmail: string ,userID:string}[]>([]);
    ...
    useEffect(() => {
        const fetchUserAds = async () => {
            ...
            try {
                ...
                const validChats = chatsData.filter(chat => chat?.ad[0].id);
                
                const uniqueAds = Array.from(new Set(validChats.map(chat => chat.ad[0]?.id)))
                    .map(id => {
                        const chat = validChats.find(c => c?.ad[0].id === id);
                        if (!chat?.ad) return null;
                        return {
  id: chat.ad[0].id,
  title: chat.ad[0].title,
  img1: chat.ad[0].img1,
  userEmail: chat.ad[0].userEmail,
  userID: chat.ad[0].userID
};
                    })
    ...
    const handleSendMessage = async () => {
        ...
        try {
            ...
            await sendMessage(
                Number(adID),
                user.email || '',
            String(selectedAd?.userEmail)    ,
                user.id,
                String(selectedAd?.id),
                message.trim()
            );
                        ...
                        {chats.length > 0 && (
                            <div className="p-4 border-b border-black-divider dark:border-dark-white-divider">
                                <h2 className="text-[0.875rem] text-black-primary dark:text-dark-white-primary text-right">
                                    {chats[0].ad[0].userEmail}
                                </h2>
                                <div className="flex justify-end items-center gap-3 mt-2">
                                    <span className="text-black-primary dark:text-dark-white-primary text-xs">{chats[0].ad[0].title}</span>
                                {chats[0].ad[0].img1 ? (<Image 
                                        alt="ad image" 
                                        src={chats[0].ad[0].img1} 
                                        className="w-12 h-12" 
                                        width={100} 
                                        height={100} 
                                    />) : (
